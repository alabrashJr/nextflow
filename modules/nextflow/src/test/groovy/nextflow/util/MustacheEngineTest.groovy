 ...
 */
class MustacheEngineTest extends Specification {

    def 'should match variable placeholder' () {

        when:
        def m1 = MustacheEngine.VAR1.matcher('{{FOO}}')
        then:
        m1.matches()
        m1.groupCount() == 3
        m1.group(1) == ''
        ...
        when:
        def m2 = MustacheEngine.VAR1.matcher('  {{this_123}} ')
        then:
        m2.matches()
        m2.groupCount() == 3
        m2.group(1) == '  '
        ...
        when:
        def m3 = MustacheEngine.VAR1.matcher('{{that-abc}}')
        then:
        m3.matches()
        m3.groupCount() == 3
        m3.group(1) == ''
        ...
        expect:
        !MustacheEngine.VAR1.matcher('{{}}').matches()
        !MustacheEngine.VAR1.matcher('${{FOO}}').matches()
        !MustacheEngine.VAR1.matcher('${FOO}').matches()
        !MustacheEngine.VAR1.matcher(' ${FOO}').matches()
        !MustacheEngine.VAR1.matcher('xxx{{FOO}}').matches()
    }

    def 'should find var placeholders' () {

        when:
        def m1 = MustacheEngine.VAR2.matcher('{{FOO}}')
        then:
        m1.find()
        m1.groupCount() == 1
        ...
        when:
        def m2 = MustacheEngine.VAR2.matcher('abc {{FOO}} {{BAR}} xyz')
        then:
        m2.find()
        m2.groupCount() == 1
        ...
        when:
        def m3 = MustacheEngine.VAR2.matcher('${{FOO}}')
        then:
        !m3.find()
    }
    ...
    def 'should replace vars' () {
        ...
        def mustache = new MustacheEngine()
